<div id="messages">
    ...
    <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="messagesDropdown">
            ...
            @foreach(Helper::messageList() as $message)
                <a class="dropdown-item d-flex align-items-center" href="{{route('message.show',$message->id)}}">
                    <div class="dropdown-list-image mr-3">
                        @if($message->photo)
                        <img class="rounded-circle" src="{{$message->photo}}" alt="profile" loading="lazy" draggable="false">
                        @else 
                        <img class="rounded-circle" src="{{asset('backend/img/avatar.png')}}" alt="default img" loading="lazy" draggable="false">
                        @endif
        ...
        <a class="dropdown-item text-center small text-gray-500" href="{{route('message.index')}}">Leer más mensajes</a>
    </div>
</div>
  ...
  $(document).ready(function() {
    ...
    Echo.channel('message')
      ...
      <a class="dropdown-item d-flex align-items-center message-item" href="${e.message.url}">
        <div class="dropdown-list-image mr-3">
          <img class="rounded-circle" src="${e.message.photo}" alt="${e.message.name}" loading="lazy" draggable="false">
        </div>
        <div class="font-weight-bold">
